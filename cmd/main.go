...
func main() {
	...
	if err = (&service.ClientService{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create service", "service", "Client")
		os.Exit(1)
	}

	if err = (&service.GatewayService{}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create service", "service", "Gateway")
		os.Exit(1)
	}
