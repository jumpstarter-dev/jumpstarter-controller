...
func LoadConfiguration(
...
) (authenticator.Token, string, Router, grpc.ServerOption, error) {
	var configmap corev1.ConfigMap
	if err := client.Get(ctx, key, &configmap); err != nil {
	...
	if !ok {
		return nil, "", nil, nil, fmt.Errorf("LoadConfiguration: missing router section")
	}
	...
	return authenticator, prefix, router, serverOptions, nil
}
